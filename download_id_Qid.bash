...
do
    echo "Calculating mapping list for ID(n1) up to " $((i * size_of_download_in_nodes))
    query=$( echo '{"statements":[{"statement":"MATCH (n1:Entity) WHERE ID(n1) > ' \
        $(( (i-1) * size_of_download_in_nodes)) ' AND ID(n1) < ' $((i * size_of_download_in_nodes)) \
        ' RETURN ID(n1) as n1, n2.id"}]}' )
    curl -H accept:application/json -H content-type:application/json \
         -d "${query}" \
         -u neo4j:caramelBunny1998\
         http://localhost:8888/db/data/transaction/commit \
         | jq -r '(.results[0]) | .columns,.data[].row | @csv' > id_mapping-wikidata-$((i * size_of_download_in_nodes)).csv
    i=$((i + 1))
done